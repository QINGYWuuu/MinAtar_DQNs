...
parser.add_argument('--id', type=str, default='TEAM_DQN', help='Experiment ID')
    ...
    def Interaction(self, step, state, act_env):
        ...
        if step < self.args.learn_start:
            ...
            if np.random.binomial(1, epsilon) == 1:
                ...
                with torch.no_grad():
                    # randomly select a head to act
                    x = self.QValue_Net_Bone(state)
                    head = np.random.randint(self.args.ensemble_size)
    ...
    def Learning(self, sample, trained_head):
        ...
        with torch.no_grad():
            ...
            if len(none_done_next_states) != 0:
                ...
                x = self.QValue_Net_Bone(none_done_next_states)
                policy_net_index = self.QValue_Net_Heads[trained_head](x).argmax(1).unsqueeze(1)
                x = self.Target_Net_Bone(none_done_next_states)
                Q_s_prime_a_prime[none_done_next_state_index] = torch.gather(self.Target_Net_Heads[trained_head](x), 1, policy_net_index).detach()


                QValue_x = self.QValue_Net_Bone(none_done_next_states)
                Target_x = self.Target_Net_Bone(none_done_next_states)

                # expected-max operator
                for head in range(self.args.ensemble_size):
                    if head == 0:
                        if head == trained_head:
                            policy_net_index = self.QValue_Net_Heads[head](QValue_x).argmax(1).unsqueeze(1)
                            Q_s_prime_a_prime[none_done_next_state_index] = torch.gather(self.Target_Net_Heads[head](Target_x), 1, policy_net_index).detach() / self.args.ensemble_size
                        else:
                            Q_s_prime_a_prime[none_done_next_state_index] = self.Target_Net_Heads[head](Target_x).detach().max(1)[0].unsqueeze(1) / self.args.ensemble_size
                    else:
                        if head == trained_head:
                            policy_net_index = self.QValue_Net_Heads[head](QValue_x).argmax(1).unsqueeze(1)
                            Q_s_prime_a_prime[none_done_next_state_index] += torch.gather(self.Target_Net_Heads[head](Target_x), 1, policy_net_index).detach() / self.args.ensemble_size

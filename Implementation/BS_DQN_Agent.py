...
parser.add_argument('--id', type=str, default='BootstrappedDQN', help='Experiment ID')
...
class DQN_Agent():
    ...
    def Train(self):
        ...
        while step < self.args.T_max:
            ...
            selected_head = np.random.randint(self.args.ensemble_size)
            while (not done) and step < self.args.T_max:
                next_state, action, reward, done = self.Interaction(step, state, self.act_env, selected_head)
    ...
    def Interaction(self, step, state, act_env, selected_head):
        ...
        if step < self.args.learn_start:
            ...
            if np.random.binomial(1, epsilon) == 1:
                ...
                with torch.no_grad():
                    ...
                    action = self.QValue_Net_Heads[selected_head](x).max(1)[1].view(1, 1)

...
parser.add_argument('--id', type=str, default='UCB_Exploration', help='Experiment ID')
        ...
        if step < self.args.learn_start:
            ...
            if np.random.binomial(1, epsilon) == 1:
                ...
                with torch.no_grad():
                    ...
                    for head in range(self.args.ensemble_size):
                        if head == 0:
                            Q_sa = self.QValue_Net_Heads[head](x)
                        else:
                            Q_sa = torch.cat((Q_sa, self.QValue_Net_Heads[head](x)))
                    Q_sa_mean = torch.mean(Q_sa, dim=0, keepdim=True)
                    Q_sa_std = torch.std(Q_sa, dim=0, keepdim=True)
                    action = (Q_sa_mean + 0.1 * Q_sa_std).max(1)[1].view(1, 1)
        ...
        with torch.no_grad():
            ...
            if len(none_done_next_states) != 0:
                if self.args.double == False:
                    ...
                    Q_s_prime_a_prime[none_done_next_state_index] = self.Target_Net_Heads[trained_head](x).detach().max(1)[0].unsqueeze(1)
                else:
...
if __name__ == '__main__':
    ...
    for args.game in ['breakout', 'freeway', 'seaquest', 'space_invaders']:

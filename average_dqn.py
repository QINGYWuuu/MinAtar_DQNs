...
def train(sample, policy_net, target_net, optimizer, target_net_all, NQ, act_dim):
    ...
    dones = torch.cat(batch_samples.is_terminal)
    ...
    avgQ = torch.zeros(len(none_done_next_states), act_dim, device=device)

    #Find average Q target
    for i in range(NQ):
        avgQ = avgQ + target_net_all[i](none_done_next_states)
    avgQ = avgQ/NQ
    Q_s_prime_a_prime = torch.zeros(len(sample), 1, device=device)

    if len(none_done_next_states) != 0:
        Q_s_prime_a_prime[none_done_next_state_index] = avgQ.detach().max(1)[0].unsqueeze(1)
    #Compute target
...
def average_dqn_agent(env, save_path, writer):
    # Get channels and number of actions specific to each game
    obs_dim = env.state_shape()[2]
    act_dim = env.num_actions()

    # Instantiate networks, optimizer, loss and buffer
    policy_net = Q_ConvNet(obs_dim, act_dim).to(device)
    #Create NQ target networks
    target_net_all = [Q_ConvNet(obs_dim, act_dim).to(device) for k in range(Q_NUM)]

    target_net = Q_ConvNet(obs_dim, act_dim).to(device)
    target_net.load_state_dict(policy_net.state_dict())

    optimizer = optim.RMSprop(policy_net.parameters(), lr=LEARNING_RATE, alpha=SQUARED_GRAD_MOMENTUM, centered=True, eps=MIN_SQUARED_GRAD)
    ...
    while step < NUM_FRAMES:
        ...
        while(not done) and step < NUM_FRAMES:
            ...
            next_state, action, reward, is_terminated = act_env(step, replay_start_size, act_dim, state, env, policy_net)
            ...
            if step % TRAINING_FREQ == 0 and sample is not None:
                ...
                train(sample, policy_net, target_net, optimizer, target_net_all, Q_NUM, act_dim)
            ...
            if policy_net_update_counter > 0 and policy_net_update_counter % TARGET_NETWORK_UPDATE_FREQ == 0:
                ...
                target_net_all[targnet_counter % Q_NUM].load_state_dict(policy_net.state_dict())
                targnet_counter += 1
...
if __name__ == '__main__':
    ...
    for game in ['breakout']:
        ...
        for run_no in range(Nruns):
            ...
            save_path = os.getcwd() + "/" + game + '/average_dqn'
            writer = SummaryWriter(save_path+'/{}_log'.format(run_no))
            env = Environment(game)
            average_dqn_agent(env, save_path, writer)